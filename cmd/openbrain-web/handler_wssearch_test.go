...
func TestChatPageRendersStructuredSearchResults(t *testing.T) {
	...
	assert.Contains(t, page, "Fast embeds your query once")
	assert.Contains(t, page, "Hybrid combines semantic similarity")
	assert.Contains(t, page, "AI Search asks the local model")
