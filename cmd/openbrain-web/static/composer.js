      ...
      host.innerHTML =
        '<div id="input-bar"><div id="composer" class="' + (mode === 'search' ? 'mode-search' : '') + '" role="search" aria-label="Search memories">' +
          ...
          '<div id="composer-toolbar"><div class="composer-tools">' +
            ...
            '<span id="search-strategy-wrap" role="group" aria-label="Search strategy"><button class="strategy-option" type="button" role="radio" data-mode="semantic" aria-checked="true" aria-label="Semantic search" data-tooltip="Semantic search combines meaning-based similarity with exact keyword matching to return ranked notes.">Semantic</button><button class="strategy-option" type="button" role="radio" data-mode="conversational" aria-checked="false" aria-label="Conversational search" data-tooltip="Conversational search finds up to 8 relevant notes, asks a local model to answer from them, and cites the notes it used.">Conversational</button></span>' +
          '</div><button id="send" disabled aria-label="Send message" title="Send message"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 19V5M6 11l6-6 6 6"/></svg></button><button id="stop-btn" style="display: none;" aria-label="Stop generation" title="Stop generation"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="6" y="6" width="12" height="12" rx="1" stroke="currentColor" stroke-width="2" fill="currentColor"/></svg></button></div></div></div>';
